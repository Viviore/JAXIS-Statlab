...
1. **Business Rules** (this document, `scope.md`) — highest authority
...
4. **Design system** (`design-system.md`) — visual decisions
...
These rules map directly from `scope.md`. Every API route, Server Action, and component that touches the relevant domain **must enforce these rules server-side**. Client-side checks are supplementary only.
...
> Per `ARCHITECTURE.md` and `scope.md`:
