...
export default function QAMessagesPage() {
  ...
  const loadThreads = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to load QA message threads:", err);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  return (
    <div className="h-full flex-1 flex flex-col min-h-0 gap-3 w-full animate-content-fade font-sans overflow-hidden">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-4 overflow-hidden">
        {/* Left Column: Assigned QA Studies Selector (4 cols on desktop, full screen on mobile when mobileView === "list") */}
        <div
            ...
            {isLoading ? (
              <>
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="p-3.5 rounded-[2px] bg-[#01142B] border border-white/10 flex flex-col gap-2 animate-pulse"
                  >
              ...
              <div className="p-6 rounded-[2px] bg-[#01142B] border border-white/10 text-center flex flex-col items-center justify-center gap-2">
                <span className="text-xs text-white/50">No QA studies match your filter</span>
          ...
          {isLoading ? (
            ...
            <Card className="h-full min-h-0 p-12 bg-[#01142B] border-white/10 flex flex-col items-center justify-center text-center gap-3">
              ...
              <div className="max-w-md">
                <h3 className="text-sm font-bold text-white">No Assigned QA Studies Found</h3>
                <p className="text-xs text-white/50 mt-1 leading-relaxed">
                  When administration assigns studies for QA review, your project communication threads will appear here automatically.
                </p>
              </div>
            </Card>
          ) : selectedThread ? (
            ...
            <Card className="h-full min-h-0 p-12 bg-[#01142B] border-white/10 flex flex-col items-center justify-center text-center text-white/50">
              ...
              <span className="text-xs text-white font-semibold">Select a research study on the left to review messages</span>
            </Card>
          )}
        </div>
      </div>
